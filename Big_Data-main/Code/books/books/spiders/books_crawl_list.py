...
class BooksCrawlSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request(url="https://books.toscrape.com/catalogue/category/books/travel_2/index.html", callback=self.parse)#link thể loại đổi

    def parse(self, response):
        book_crawl = response.xpath("//ol[@class='row']/li/article[@class='product_pod']")#không đổi
        for book in book_crawl:
            item = BooksItem()
            item["title"] = book.xpath('.//h3/a/@title').get()#item vs xpath thì cmt nếu k muốn lấy
    ...
    def detailBooks(self, response):
        item = response.meta["item"]#không đổi
        #description
