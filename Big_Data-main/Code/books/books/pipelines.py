...
class ToCSV:
    def __init__(self):
        self.file = open('books.csv', 'w', newline='', encoding='utf-8')#tên file cào về
    ...
    def process_item(self, item, spider):
        ...
        #nếu đóng xpath bên crawl thì đóng item bên này
...
class MySQLPipeline:
    def __init__(self):
        ...
        self.cursor.execute("SHOW DATABASES LIKE 'books_db'")#tên database
        db_exists = self.cursor.fetchone()
        # Nếu cơ sở dữ liệu tồn tại, xóa nó đi
        if db_exists:
            self.cursor.execute("DROP DATABASE books_db")#sửa
        # Tạo cơ sở dữ liệu mới
        self.cursor.execute("CREATE DATABASE books_db")#sửa
        # Sử dụng cơ sở dữ liệu booksdb
        self.cursor.execute("USE books_db")#sửa
        ...
        """)
        #cột insert xóa cùng với cmt trên tạo bảng- dưới values xóa % - cmt item dưới
    def process_item(self, item, spider):
        self.cursor.execute('''
...
class MongoDBPipeline:
    def __init__(self):
        self.client = pymongo.MongoClient('mongodb+srv://thuan123:thuan123@thuan.xhlbznz.mongodb.net/?retryWrites=true&w=majority&appName=thuan')
